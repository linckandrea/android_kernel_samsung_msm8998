...
static void mmc_card_error_logging(struct mmc_card *card, struct mmc_blk_request *brq, u32 status)
{
	 ...
	 */
	if (noti && mmc_card_sd(card) && card->host->sdcard_uevent) {
		error = card->host->sdcard_uevent(card);
		if (error)
			pr_err("%s: Failed to Send Uevent with err %d\n",
					mmc_hostname(card->host), error);
		else
	...
	struct mmc_blk_request *brq, int *ecc_err, int *gen_err)
{
	...
	if (err) {
		bool prev_SDR104 = (card->sd_bus_speed == UHS_SDR104_BUS_SPEED) ? true : false;
		if (card->err_in_sdr104)
			return ERR_RETRY;
		/* Check if the card is removed */
		if (mmc_detect_card_removed(card->host))
			return ERR_NOMEDIUM;
		if (prev_SDR104 && !card->host->sdr104_wa)
			return ERR_RETRY;
		else
			return ERR_ABORT;
	}
