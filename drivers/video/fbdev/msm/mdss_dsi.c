		...
		u64 new_clk_rate)
{
	...
	pinfo->clk_rate = new_clk_rate;
	mdss_dsi_calc_bitclk (&ctrl_pdata->panel_data.panel_info, pinfo->mipi.frame_rate);
...
static int mdss_dsi_parse_hw_cfg(struct platform_device *pdev, char *pan_cfg)
{
	...
	pr_err("%s: DSI h/w configuration is %d\n", __func__,
		sdata->hw_config);
...
#if defined(CONFIG_FB_MSM_MDSS_SAMSUNG)
	ctrl_pdata->tconrdy_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "qcom,platform-tconrdy-gpio", 0);

	if (!gpio_is_valid(ctrl_pdata->tconrdy_gpio))
		pr_err("%s:%d, tconrdy gpio not specified\n",
						__func__, __LINE__);
#endif
